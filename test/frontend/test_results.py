...
class Tests(saliweb.test.TestCase):
    ...
    def test_ok_job(self):
        ...
        with saliweb.test.make_frontend_job('testjob2') as j:
            ...
            r = re.compile('Receptor.*Ligand.*Score Type.*test\.pdb.*'
                           'test\.mol2.*PoseScore\.lib.*<td>1</td>.*'
                           '<td>\-34\.62</td>.*</tr>.*\-20\.02.*\-25\.75.*'
                           'show next 20.*score\.list.*Download output file',
            ...
            r = re.compile('Receptor.*Ligand.*Score Type.*test\.pdb.*'
                           'test\.mol2.*PoseScore\.lib.*<td>25</td>.*'
                           '<td>\-34\.62</td>.*</tr>.*\-20\.02.*\-25\.75.*'
                           'show prev 20.*score\.list.*Download output file',
                           re.MULTILINE | re.DOTALL)
            self.assertRegexpMatches(rv.data, r)
    ...
    def test_failed_job(self):
        ...
        with saliweb.test.make_frontend_job('testjob3') as j:
            ...
            self.assertIn('No output file was produced', rv.data)
